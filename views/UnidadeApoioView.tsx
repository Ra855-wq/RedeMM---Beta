...
export const UnidadeApoioView: React.FC = () => {
  ...
  return (
    <div className="h-[calc(100vh-140px)] flex flex-col gap-8 animate-in fade-in duration-700">
      {/* Search Bar */}
      <div className="bg-white/50 backdrop-blur-xl p-6 rounded-[3rem] border border-slate-100 shadow-surgical flex flex-col md:flex-row gap-4 items-center">
        <div className="flex-1 w-full relative">
          <input 
            ...
            className="w-full pl-8 pr-8 py-5 bg-white border border-slate-50 rounded-full focus:outline-none focus:ring-4 focus:ring-accent-500/5 font-bold text-slate-800 transition-all text-sm shadow-inner"
          />
        </div>
        <div className="flex gap-4 w-full md:w-auto shrink-0">
          <button 
            ...
            disabled={loading}
            className="flex-1 md:flex-none bg-neutral-900 text-white px-10 py-5 rounded-full font-black text-xs uppercase tracking-widest hover:bg-neutral-800 transition-all flex items-center justify-center gap-3 shadow-xl active:scale-95 disabled:opacity-50"
          >
            {loading ? <Loader2 className="animate-spin" size={18} /> : <Target size={18} />}
            MAPEAR REDE
          </button>
          <button onClick={handleGetLocation} className="p-5 bg-white text-neutral-800 border border-slate-50 rounded-full hover:bg-slate-50 hover:text-accent-600 transition-all shadow-surgical active:scale-95">
            <Crosshair size={20} />
          </button>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col lg:flex-row gap-8 min-h-0">
        {/* Results List */}
        <div className="w-full lg:w-[400px] bg-white rounded-[3rem] border border-slate-100 shadow-surgical flex flex-col overflow-hidden shrink-0">
           <div className="p-8 border-b border-slate-50">
              <h3 className="font-black text-neutral-900 text-[10px] uppercase tracking-[0.3em] flex items-center gap-2">
           ...
           <div className="flex-1 overflow-y-auto custom-scroll p-6 space-y-4">
              {results.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-center px-10">
                  ...
                  <p className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-300 italic leading-relaxed">
                    INICIE O MAPEAMENTO PARA VISUALIZAR A REDE.
                  </p>
                </div>
              ) : results.map((r, i) => (
                <div key={r.id} className="p-6 bg-slate-50/50 rounded-[2rem] border border-slate-100 hover:border-accent-400 hover:bg-white hover:shadow-surgical transition-all duration-300 group cursor-pointer" onClick={() => mapRef.current?.setView([r.lat, r.lng], 17)}>
                   <div className="flex justify-between items-start mb-4">
                      <div className="w-10 h-10 bg-white rounded-xl border border-slate-100 flex items-center justify-center text-accent-600 shadow-sm group-hover:bg-neutral-900 group-hover:text-white transition-all">
                        <Building2 size={20} />
                      </div>
                      <button onClick={(e) => { e.stopPropagation(); speakText(i, r); }} className={`p-3 rounded-xl transition-all ${speakingId === i ? 'bg-accent-600 text-white animate-pulse' : 'bg-white text-slate-300 hover:text-accent-600 shadow-sm'}`}>
                        <Volume2 size={16} />
                      </button>
                   </div>
                   <h4 className="font-black text-slate-800 text-sm group-hover:text-accent-600 transition-colors mb-4 truncate">{r.title}</h4>
                   <div className="flex items-center justify-between pt-4 border-t border-slate-100/50">
                      <span className="text-[8px] font-black text-slate-300 uppercase tracking-widest flex items-center gap-2"><Hash size={10} /> REDEMM #{1000 + i}</span>
                      <a href={r.uri} target="_blank" className="p-2 text-accent-600 hover:scale-110 transition-transform"><Navigation size={16} /></a>
                   </div>
                </div>
              ))}
           </div>
        ...
        <div className="flex-1 bg-slate-100 rounded-[3.5rem] border-4 border-white shadow-surgical-xl overflow-hidden relative group">
          <div ref={mapContainerRef} className="w-full h-full grayscale-[0.1] hover:grayscale-0 transition-all duration-1000" />
          
          {/* Integrated HUD Row */}
          <div className="absolute top-6 left-6 right-6 z-20 flex gap-4 pointer-events-none items-center">
             <div className="bg-neutral-900/95 backdrop-blur-xl px-6 py-4 rounded-2xl shadow-2xl flex items-center gap-3 border border-white/10 pointer-events-auto">
                <div className="w-1 h-3 bg-accent-500 rounded-full shadow-[0_0_10px_#3b82f6]"></div>
                <span className="text-[9px] font-black text-white uppercase tracking-[0.3em]">OPTIMIZED CLUSTER ENGINE</span>
             </div>
             ...
             <div className="flex gap-2 overflow-x-auto no-scrollbar pointer-events-auto flex-1">
                {quickFilters.map((f, i) => (
                  <button 
                    ...
                    className="px-6 py-4 bg-white/95 backdrop-blur-xl rounded-2xl text-[9px] font-black text-neutral-900 uppercase tracking-widest border border-slate-100 shadow-2xl hover:bg-accent-600 hover:text-white transition-all flex items-center gap-2 active:scale-95 whitespace-nowrap group/btn"
                  >
                    <f.icon size={12} className="group-hover/btn:text-white" /> {f.label}
                  </button>
                ))}
             </div>
          </div>
