...
import { BrainCircuit, Send, Loader2, Sparkles, User, Bot, Mic, Volume2, Lightbulb, Zap } from 'lucide-react';
...
export const AssistantView: React.FC = () => {
  ...
  const [useDeepThinking, setUseDeepThinking] = useState(false); // Default to Flash for speed
  ...
  const speakText = async (text: string) => {
    ...
    try {
      // Pré-inicializa o contexto para evitar latência de retomada
      ...
      // Limita o texto para acelerar a geração do TTS
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-140px)] max-w-5xl mx-auto animate-in fade-in duration-500 relative">
      <div className="bg-white/90 backdrop-blur-2xl rounded-[2.5rem] border border-slate-200 p-6 flex flex-col sm:flex-row items-center justify-between shadow-2xl mb-6 gap-6 floating-card">
